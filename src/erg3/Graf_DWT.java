 ...
 * Autor: Cesar Peña
 */
public class Graf_DWT extends javax.swing.JPanel implements MouseListener, MouseMotionListener {
    
    private DWT_Form1 form;
    private double divT,divV,ScalmV,Scalms,maximo,minimo,dv;
    private double s_ent1[]=null;
    int     offsetX, offsetY;   
    boolean dragging; 
    int     x0,y0;
    double  mx=12.231;
    ...
    public Graf_DWT(DWT_Form1 fr, String ojo, String cuad) {
         this.form = fr;
         addMouseListener(this);
         addMouseMotionListener(this);
         this.ojo = ojo;
         this.Cd = cuad;
         this.s_ent1 = form.Ret_S();
         offsetX=x0;
         this.x0=0;
         f1= new Font("Fuente1",Font.BOLD,10);
          for(int i=0;i<s_ent1.length;i++){
           if(maximo >=Math.abs(s_ent1[i])) maximo = maximo;
           else maximo = Math.abs(s_ent1[i]);
           if(minimo <=Math.abs(s_ent1[i])) minimo = minimo;
           else minimo = Math.abs(s_ent1[i]);
       }
        dv=maximo-minimo;
    }
    ...
    public void paintComponent(Graphics g1) {
     ...
     public void Graficar(Graphics g1,int xi){
      ...
      if(form.Ret_opc()=="Aproximación"){
       ...
       g.drawString("Señal Ojo "+ojo+"- Cuad: "+Cd,this.getWidth()-161,20);          
       for(int x=0;x<s_ent1.length;x++){
         g.setColor(Color.YELLOW);
         g.draw(new Rectangle2D.Double((x+xi/mx)*divT,this.getHeight()/2+(s_ent1[x])*(this.getHeight()-200)*divV/15,2,2));
         
         for(int k=2;k<s_ent1.length;k+=4){
            if((x+1)==k){
         u++;       
         g.setColor(Color.WHITE);
         ...
         g.draw(new Line2D.Double((x+xi/mx)*divT,(this.getHeight()/2+(s_ent1[x])*(this.getHeight()-200)*divV/15),
         ((x+xi/mx)-1)*divT,(this.getHeight()/2+(s_ent1[x-1])*(this.getHeight()-200)*divV/15)));   
                }
        }
