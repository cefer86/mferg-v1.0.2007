 ...
 * Autor: Cesar Peña
 */
public class Graf_DWT_D extends javax.swing.JPanel implements MouseListener, MouseMotionListener{
    
    private DWT_Form1 form;
    private double divT,divV,ScalmV,Scalms,maximo,minimo,dv,maximo1,minimo1,dv1;
    private double apx2[]=null,det2[]=null;
    int     offsetX, offsetY;   
    boolean dragging; 
    int     x0,y0;
    double  mx=12.231;
    ...
    public Graf_DWT_D(DWT_Form1 fr, String cuad) {
         this.form = fr;
         addMouseListener(this);
         addMouseMotionListener(this);
         this.Cd = cuad;
         this.apx2 = form.Ret_A2();
         this.det2 = form.Ret_D2();
         offsetX=x0;
         this.x0=0;
         f1= new Font("Fuente1",Font.BOLD,10);
         for(int i=0;i<apx2.length;i++){
           if(maximo >=Math.abs(apx2[i])) maximo = maximo;
           else maximo = Math.abs(apx2[i]);
           if(minimo <=Math.abs(apx2[i])) minimo = minimo;
           else minimo = Math.abs(apx2[i]);
       }
        dv=7.5*(maximo-minimo);
        for(int i=0;i<det2.length;i++){
           if(maximo1 >=Math.abs(det2[i])) maximo1 = maximo1;
           else maximo1 = Math.abs(det2[i]);
           if(minimo1 <=Math.abs(det2[i])) minimo1 = minimo1;
           else minimo1 = Math.abs(det2[i]);
       }
    ...
    public void paintComponent(Graphics g1) {
     ...
     public void Graficar(Graphics g1, int xi){
      ...
      if(form.Ret_opc()=="Aproximación"){
        ...
        g.drawString("CA[2] - Cuad: "+Cd,this.getWidth()-100,20);             
       for(int x=0;x<apx2.length;x++){
             g.setColor(Color.GREEN);
             g.draw(new Rectangle2D.Double((x+xi/mx)*divT,this.getHeight()/2+(apx2[x])*(this.getHeight()-200)*divV/15,2,2));

           for(int k=2;k<apx2.length;k+=4){
            if((x+1)==k){
             u++;       
             g.setColor(Color.WHITE);
             ...
             g.draw(new Line2D.Double((x+xi/mx)*divT,(this.getHeight()/2+(apx2[x])*(this.getHeight()-200)*divV/15),
            ((x+xi/mx)-1)*divT,(this.getHeight()/2+(apx2[x-1])*(this.getHeight()-200)*divV/15)));   
             }
          }
        ...
        g.setColor(Color.GREEN);  
        g.setFont(f1);
        g.drawString("CD[2] - Cuad: "+Cd,this.getWidth()-100,20);    
        for(int x=0;x<det2.length;x++){
             g.setColor(Color.YELLOW);
             g.draw(new Rectangle2D.Double((x+xi/mx)*divT,this.getHeight()/2+(det2[x])*(this.getHeight()-200)*divV/15,2,2));

             for(int k=2;k<det2.length;k+=4){
            if((x+1)==k){
             u++;       
             g.setColor(Color.WHITE);
             g.fill(new Rectangle2D.Double((x+xi/mx)*divT,this.getHeight()/2,3,3));  
             ...
             if(x!=0){
             g.setColor(Color.GREEN);    
             g.draw(new Line2D.Double((x+xi/mx)*divT,(this.getHeight()/2+(det2[x])*(this.getHeight()-200)*divV/15),
            ((x+xi/mx)-1)*divT,(this.getHeight()/2+(det2[x-1])*(this.getHeight()-200)*divV/15)));   
             }
          }
