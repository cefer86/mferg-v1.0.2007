 ...
 *
 */
public class k2 extends javax.swing.JPanel implements MouseListener, MouseMotionListener {
    ...
    private double Kernel2corr[]=null;
    private double Kernel2[]=null;
    ...
    public k2(Kernels fr, Test0 fr1){
        ...
        this.Scalms =0.0;
        if(!fr1.importD){
        this.Kernel2 = form.RetK2();
        this.correlacion = new CorrCruz(Kernel2,Kernel2.length,form.s_entrada);
        this.Kernel2corr = correlacion.Corr2();
        this.ds = (int)this.Kernel2corr.length/103;
        }else if(fr1.importD){
            this.Kernel2corr = fr1.v_k2;
            this.Kernel2 = fr1.k2;
            this.ds = (int)this.Kernel2corr.length/103;
        }
        ...
        offsetX=x0; 
        this.x0=0;
        for(int i=0;i<Kernel2corr.length;i++){
           if(maximo >=Math.abs(Kernel2corr[i])) maximo = maximo;
           else maximo = Math.abs(Kernel2corr[i]);
           if(minimo <=Math.abs(Kernel2corr[i])) minimo = minimo;
           else minimo = Math.abs(Kernel2corr[i]);
       }
        dv=maximo-minimo;
    }
    ...
    public void Graficar(Graphics g1, int xi){
       ...
       double dx=0.1;
       ScalmV = form1.RetmVk2();
       Scalms = form1.RetmSk2();
      ...
      for(int w=0;w<103;w++){
          if(form1.RetHex().equals("h"+w)){
              ...
              for(int x=w*ds;x<lo;x++){
                ...
                g.draw(new Rectangle2D.Double((dx+xi/mx)*divT,this.getHeight()/2+(Kernel2corr[x])*(this.getHeight()-200)*divV/15,1,1));
                if(x!=0){
                g.setColor(Color.GREEN);
                g.draw(new Line2D.Double((dx+xi/mx)*divT,(this.getHeight()/2+(Kernel2corr[x])*(this.getHeight()-200)*divV/15),
                ((dx+xi/mx)-0.1*form.RetBaseTiempo_delta_r())*divT,(this.getHeight()/2+(Kernel2corr[x-1])*(this.getHeight()-200)*divV/15)));   
             if(!form.importD){  
              ...
              }else if(form1.RetOpMot()=="Ventana" && Kernel2!=null){
              dx=0.1;
              int d=1;
              ...
              for(int x=w*ds;x<(w+1)*ds;x++){
                ...
                if(x!=0){
                    if(x==d*correlacion.RetIndice_time()+w*ds){
                       if(Kernel2[x*4/correlacion.RetIndice_time()]==1)
                       g.draw(new Rectangle2D.Double((dx+xi/mx)*divT,this.getHeight()/2-9,4,9));   
                       else if(Kernel2[x*4/correlacion.RetIndice_time()]==-1)
                  ...
                  if(x!=0){
                    if(x==d*form.indice_timeImD){
                       if(Kernel2[x*4/form.indice_timeImD]==1)
                       g.draw(new Rectangle2D.Double((dx+xi/mx)*divT,this.getHeight()/2-9,4,9));   
                       else if(Kernel2[x*4/form.indice_timeImD]==-1)
                       g.draw(new Rectangle2D.Double((dx+xi/mx)*divT,this.getHeight()/2,4,9)); 
                       d++;
                    }
                     dx=dx+0.1*form.RetBaseTiempo_delta_r();      
              ...
              g.setColor(Color.GREEN);
              g.setFont(f2);
              g.drawString("1S - Kernel Ord. 2: Hex "+w,this.getWidth()-150.5f,this.getHeight()-9.2f);
    ...
    public double[] RetK2wvf(){
        return Kernel2corr;
    }
    public double[] RetK2_binaryCan(){
        return Kernel2;
    }
}
