...
public class k1 extends javax.swing.JPanel implements MouseListener, MouseMotionListener{
    ...
    private double Kernel1corr[]=null;
    private double Kernel1[]=null;
    
    public k1() {
        initComponents();
    }
    public k1(Kernels fr,Test0 fr1){
        ...
        this.Scalms =0.0;
        if(!fr1.importD){
        this.Kernel1 = form.RetK1();
        this.correlacion = new CorrCruz(Kernel1,Kernel1.length,form.s_entrada);
        this.Kernel1corr = correlacion.Corr2();
        this.ds = (int)this.Kernel1corr.length/103;
        }else if(fr1.importD) {
            this.Kernel1corr = fr1.v_k1;
            this.Kernel1 = fr1.k1;
            this.ds = (int)this.Kernel1corr.length/103;
        }
        ...
        offsetX=x0; 
        this.x0=0;
        for(int i=0;i<Kernel1corr.length;i++){
           if(maximo >=Math.abs(Kernel1corr[i])) maximo = maximo;
           else maximo = Math.abs(Kernel1corr[i]);
           if(minimo <=Math.abs(Kernel1corr[i])) minimo = minimo;
           else minimo = Math.abs(Kernel1corr[i]);
       }
        dv=maximo-minimo;
    ...
    public void Graficar(Graphics g1,int xi){
       ...
       double dx=0.1;
       ScalmV = form1.RetmVk1();
       Scalms = form1.RetmSk1();
      ...
      for(int w=0;w<103;w++){
          if(form1.RetHex().equals("h"+w)){
              ...
              for(int x=w*ds;x<lo;x++){
                ...
                g.draw(new Rectangle2D.Double((dx+xi/mx)*divT,this.getHeight()/2+(Kernel1corr[x])*(this.getHeight()-200)*divV/15,1,1));
                if(x!=0){
                g.setColor(Color.GREEN);
                g.draw(new Line2D.Double((dx+xi/mx)*divT,(this.getHeight()/2+(Kernel1corr[x])*(this.getHeight()-200)*divV/15),
                ((dx+xi/mx)-0.1*form.RetBaseTiempo_delta_r())*divT,(this.getHeight()/2+(Kernel1corr[x-1])*(this.getHeight()-200)*divV/15)));
              if(!form.importD){  
              ...
              }else if(form1.RetOpMot()=="Ventana" && Kernel1!=null){
              ...
              for(int x=w*ds;x<(w+1)*ds;x++){
                ...
                if(x!=0){
                    if(x==d*correlacion.RetIndice_time()+w*ds){
                       if(Kernel1[x*4/correlacion.RetIndice_time()]==1)
                       g.draw(new Rectangle2D.Double((dx+xi/mx)*divT,this.getHeight()/2-9,4,9));   
                       else if(Kernel1[x*4/correlacion.RetIndice_time()]==-1)
                  ...
                  if(x!=0){
                    if(x==d*form.indice_timeImD){
                       if(Kernel1[x*4/form.indice_timeImD]==1)
                       g.draw(new Rectangle2D.Double((dx+xi/mx)*divT,this.getHeight()/2-9,4,9));   
                       else if(Kernel1[x*4/form.indice_timeImD]==-1)
                       g.draw(new Rectangle2D.Double((dx+xi/mx)*divT,this.getHeight()/2,4,9)); 
                       d++;
                    }
                     dx=dx+0.1*form.RetBaseTiempo_delta_r();      
             ...
             g.setColor(Color.GREEN); 
             g.setFont(f2);
             g.drawString("Kernel Ord. 1: Hex "+w,this.getWidth()-130.5f,this.getHeight()-9.2f);
          } 
      }
